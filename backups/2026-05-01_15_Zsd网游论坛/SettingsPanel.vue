  ...
  <div class="absolute inset-0 z-10 overflow-y-auto p-3" :style="{ backgroundColor: 'var(--f-bg)' }">
    ...
    <div class="space-y-3">
      ...
      <div>
        ...
        <input v-model="store.settings.ZsectionAName" class="w-full text-xs px-2 py-1.5 rounded outline-none border focus:border-[var(--f-accent)]" :style="inputStyle" placeholder="如：现实讨论、日常闲聊、新闻频道" />
      </div>
      <div>
        <label class="text-[11px] text-[var(--f-text-secondary)] block mb-1">论坛B 名称</label>
        <input v-model="store.settings.ZsectionBName" class="w-full text-xs px-2 py-1.5 rounded outline-none border focus:border-[var(--f-accent)]" :style="inputStyle" placeholder="如：游戏中论坛、公会大厅、副本组队" />
      </div>
      ...
      <div class="flex items-center gap-2">
        ...
        <div>
          ...
          <p class="text-[10px]" :style="{ color: 'var(--f-text-muted)' }">关闭后，AI生成帖子/评论时只使用论坛自己的提示词，不携带角色卡、世界书等酒馆预设内容</p>
        </div>
      </div>
      ...
      <div>
        ...
        <textarea v-model="store.settings.Zprompt_A" rows="4" class="w-full text-xs px-2 py-1.5 rounded outline-none border focus:border-[var(--f-accent)] resize-none" :style="inputStyle"></textarea>
      </div>
      <div>
        <label class="text-[11px] text-[var(--f-text-secondary)] block mb-1">{{ sectionBName }} - 系统提示词</label>
        <textarea v-model="store.settings.Zprompt_B" rows="4" class="w-full text-xs px-2 py-1.5 rounded outline-none border focus:border-[var(--f-accent)] resize-none" :style="inputStyle"></textarea>
      </div>
      <div>
...
const sectionAName = computed(() => store.settings.ZsectionAName || '论坛A');
const sectionBName = computed(() => store.settings.ZsectionBName || '论坛B');
...
const defaultPrompts = computed(() => [
  { label: `${sectionAName.value} 默认提示词`, content: defaultPromptA },
  { label: `${sectionBName.value} 默认提示词`, content: defaultPromptB },
