...
function parseRawPosts(rawPosts: any[], section: 'A' | 'B'): ForumPost[] {
...
export async function generatePosts(section: 'A' | 'B', topic?: string) {
  ...
  return parseRawPosts(parsed.posts || [], section);
}
...
以JSON格式返回，结构为 { "A": { "posts": [...] }, "B": { "posts": [...] } }。每个帖子包含 title、content、authorId、timestamp、comments 字段。`;
  ...
  const parsed = JSON.parse(result as string);
  return {
    A: parseRawPosts(parsed.A?.posts || [], 'A'),
    B: parseRawPosts(parsed.B?.posts || [], 'B'),
  };
}
